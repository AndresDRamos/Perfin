...
| dashboard | docs/modules/ledger.md (timeline + income_schedule); docs/modules/accounts.md (Dashboard section); docs/modules/budgets.md (Dashboard surface); docs/database/data-dictionary.md | docs/plans/* (history) | Touches the balance-timeline projection rules (dedupe schedule/projected/burn), the `income_schedule` recurrence semantics (quincenal = 15 y fin de mes), or the payday confirmation flow? All series math is pure (`src/domain/{recurrence,timeline}.ts`) — change it there with tests, never in the action. |
